...
/**
 * Returns the collision key for a local path: Unicode NFC normalization plus
 * full case folding (sharp s, Greek final sigma and variant letters, ligatures,
 * dotted I, ...). Two paths collide when their keys are equal, which catches a
 * case-only rename across OSes while preserving the original display name. The
 * table embeds the same fold data Go derives from cases.Fold().
 */
export function portablePathKey(path: string): string {
  let out = ''
  for (const ch of path.normalize('NFC')) {
    out += CASE_FOLD[ch] ?? ch
  }
  return out
}
